  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      >
        ...
        <div className="relative flex flex-col md:flex-row md:items-center gap-6">
          <div className="flex-1">
            <div
              className="inline-flex items-center gap-2 px-3 py-1 rounded-lg text-xs font-semibold mb-4"
              style={{ background: "rgba(20,184,166,0.2)", color: "#5eead4", border: "1px solid rgba(20,184,166,0.3)" }}
            >
              <Flame size={12} /> Faculty Dashboard
            </div>
            <h1 className="text-2xl md:text-3xl font-bold leading-tight mb-2" style={{ fontFamily: "'DM Serif Display', serif" }}>
              Welcome back, {facultyProfile.name.split(" ")[1]} 
            </h1>
            <p className="text-sm opacity-70 max-w-md leading-relaxed">
              You have <span className="font-semibold" style={{ color: "#5eead4" }}>{dashboardStats.pending} events</span> awaiting admin approval and{" "}
              <span className="font-semibold" style={{ color: "#5eead4" }}>{dashboardStats.upcomingThisMonth} upcoming</span> this month.
            </p>
          ...
          <div className="grid grid-cols-2 gap-3 md:w-56 flex-shrink-0">
            {[
              { label: "Events Created", val: dashboardStats.totalEvents, icon: ClipboardList },
              { label: "Registrations", val: dashboardStats.totalRegistrations, icon: Users },
              { label: "Avg Fill Rate", val: `${dashboardStats.avgFillRate}%`, icon: TrendingUp },
              { label: "Rating", val: `${facultyProfile.rating}`, icon: Star },
            ].map(({ label, val, icon: Icon }) => (
              <div key={label} className="rounded-2xl p-3 text-center" style={{ background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.1)" }}>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {(["approved", "pending", "draft", "rejected"] as const).map((status, i) => {
          ...
          const counts = {
            ...
            draft: dashboardStats.drafts,
            rejected: dashboardStats.rejected,
          };
          return (
            <div key={status} className="card p-5 relative overflow-hidden" style={{ animationDelay: `${i * 0.08}s` }}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 card overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-slate-50">
            <div>
              <h3 className="font-bold text-slate-900 text-sm flex items-center gap-1.5" style={{ fontFamily: "'DM Serif Display', serif" }}>
          ...
          <div className="divide-y divide-slate-50">
            {upcoming.map((ev) => {
              ...
              const hot = fill > 80;
              const BannerIcon = BANNER_ICONS[ev.banner];
              return (
                    ...
                    <div className="flex items-center gap-2 mb-0.5">
                      ...
                      {hot && (
                        <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-md bg-red-50 text-red-600 border border-red-200 flex-shrink-0">
                          <Flame size={10} className="inline mr-1" />Hot
                        </span>
                      )}
                    </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          { icon: Rocket, title: "Create New Event", sub: "Start building your next event", href: "/create-event", bg: "from-teal-500 to-teal-700" },
          { icon: BarChart3, title: "View All Events", sub: "Track registrations & statuses", href: "/my-events", bg: "from-blue-500 to-blue-700" },
          { icon: Trophy, title: "Top Performers", sub: `${facultyProfile.totalParticipants} total participants`, href: "/my-events", bg: "from-amber-500 to-orange-600" },
        ].map(({ icon: Icon, title, sub, href, bg }) => (
          <Link key={title} href={href} className="group card p-5 flex items-center gap-4 hover:shadow-md transition-all cursor-pointer">
